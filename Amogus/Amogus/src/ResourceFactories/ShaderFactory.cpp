...
namespace ShaderFactory
{
	...
	{
		const GLuint vertexID = CompileShaderFromFile("Assets/Shaders/" + vertexPath, GL_VERTEX_SHADER);
		const GLuint fragmentID = CompileShaderFromFile("Assets/Shaders/" + fragmentPath, GL_FRAGMENT_SHADER);
		const GLuint geometryID = !geometryPath.empty() ? CompileShaderFromFile("Assets/Shaders/" + geometryPath, GL_GEOMETRY_SHADER) : INVALID_SHADER_ID;
		...
		return new Shader(name, programID, "Assets/Shaders/" + vertexPath, "Assets/Shaders/" + fragmentPath, "Assets/Shaders/" + geometryPath);
	}
	...
	{
		const GLuint computeID = CompileShaderFromFile("Assets/Shaders/" + computePath, GL_COMPUTE_SHADER);
		...
		return new ComputeShader(name, programID, numGroupsX, numGroupsY, numGroupsZ, "Assets/Shaders/" + computePath);
	}
